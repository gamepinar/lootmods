...
function Home() {
  ...
  return (
    <>
      ...
      <section className="donation-mini-box glass glow-cyan">
        ...
        {selectedMethod && (
          <>
            ...
            <div style={{marginTop: '0.8rem', padding: '0 10%'}}>
               <input 
                type="text" 
                placeholder="Name (for the Honor Wall)" 
                className="search-input glass" 
                style={{fontSize: '0.8rem', padding: '0.4rem 1rem', textAlign: 'center', border: donorName.trim() ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(168, 85, 247, 0.4)'}}
                value={donorName}
      ...
      <section style={{padding: '1rem 8%', textAlign: 'center'}}>
        <h2 style={{marginBottom: '1rem', fontSize: '1.6rem', color: '#10b981'}}>HONOR WALL</h2>
        <div className="glass" style={{maxWidth: '600px', margin: '0 auto', padding: '1rem', borderRadius: '1.5rem'}}>
          {donations.length > 0 ? (
      ...
      <footer style={{padding: '0.5rem 10%', textAlign: 'center', opacity: 0.8}}>
        <div className="glass" style={{padding: '1.5rem', borderRadius: '2rem', border: '1px solid rgba(255,255,255,0.05)', position: 'relative'}}>
          <button onClick={() => setIsSpanish(!isSpanish)} className="translate-btn" style={{position: 'absolute', top: '1rem', right: '1rem'}}>{isSpanish ? '🇺🇸 English' : '🇪🇸 Español'}</button>
          <h3 style={{marginBottom: '1rem', color: 'var(--accent-cyan)', fontSize: '1.2rem'}}>{isSpanish ? 'SOMOS GAMERS, IGUAL QUE VOS.' : 'WE ARE GAMERS, JUST LIKE YOU.'}</h3>
          <p style={{lineHeight: '1.6', fontSize: '0.95rem', color: 'var(--text-secondary)', maxWidth: '800px', margin: '0 auto'}}>
          ...
          <div style={{marginTop: '1rem', fontStyle: 'italic', color: 'var(--accent-purple)', fontWeight: '600', fontSize: '0.9rem'}}>
        ...
        <p style={{marginTop: '1.5rem', fontSize: '0.8rem', opacity: 0.4}}>© {new Date().getFullYear()} LOOTMODS - Preserving the Gaming Heritage</p>
      </footer>
    </>
  );
}
